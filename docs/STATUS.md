...
**Live test suite:** 2480 collected

**Islands by category** (63 total — real code, no production caller):

- **facade-only** (imported only by a package `__init__` re-export — false liveness): `gen.discovery.active_resolution`, `gen.discovery.assumption_annihilator`, `gen.discovery.cosmic_insight`, `gen.discovery.first_principles`, `gen.discovery.reality_fork`, `gen.discovery.surrogate`, `gen.discovery.universe_bridge`, `gen.integration.audited_run`, `gen.integration.drift`, `gen.ledger.postgres`, `gen.mcp.adapter`, `gen.tools.wikidata`, `gen.wissensbasis.evidence`
