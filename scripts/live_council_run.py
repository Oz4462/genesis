...
Two paths, same gate:
  ...
  * Without ``--live`` (and the default when the live CLIs are unreachable, e.g. inside an agent
    ...
    LIVE from grok + Claude on 2026-06-19 — the canonical ``CAPTURED_PROPOSALS`` in ``symbiosis`` (one
    source of truth, also what ``gen --mode council`` replays) — so the gating result is reproducible
    and the proposals are genuinely the models', not invented.
...
from gen.discovery.symbiosis import (  # noqa: E402
    CAPTURED_PROPOSALS,
    council_discover,
    default_council,
    scripted_council,
...
def _proposers_for(case_name: str, live: bool):
    # Offline default replays the canonical captured proposals (single source of truth in symbiosis);
    # --live swaps in the real grok + claude CLIs.
    return default_council() if live else scripted_council(CAPTURED_PROPOSALS[case_name])
