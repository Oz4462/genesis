...
def test_build_live_wires_models_into_config_and_deps():
    ...
    # formula-aware backend (DLMF/CODATA authoritative sources) is wired third; arXiv +
    # OpenAlex add keyless preprint / scholarly-graph breadth. PatentsView is NOT in the
    # default set (it needs an X-Api-Key — registered only when PATENTSVIEW_API_KEY is set).
    assert [b.name for b in deps.backends] == [
        "wikipedia", "semantic_scholar", "formula", "arxiv", "openalex",
    ]
