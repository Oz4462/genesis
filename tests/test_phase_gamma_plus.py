...
def test_derive_goal_from_spec_real_from_quantities_and_measurands():
    """Derive must use real q.ids/units from spec (prefer measurand), produce valid goal for γ+."""
    # shaft spec has mix of measurand and non (see _q + _shaft_spec)
    spec = _shaft_spec("derive-test", diameter=22.0, cost=7.5)
    goal = derive_goal_from_spec(spec, "derive-g1")
    assert isinstance(goal, InverseDesignGoal)
    assert goal.id == "derive-g1"
    assert len(goal.objectives) >= 1
    assert len(goal.objectives) <= 3
    qids = {q.id for q in spec.quantities}
    for obj in goal.objectives:
        assert obj.quantity_id in qids, f"obj qid {obj.quantity_id} not in spec qs {qids}"
        # unit must match a q's
        matching = [q for q in spec.quantities if q.id == obj.quantity_id]
        assert matching and obj.unit == matching[0].unit
    # since some measurand present, at least one obj should come from prioritized
    meas_qs = {q.id for q in spec.quantities if q.measurand}
    if meas_qs:
        assert any(o.quantity_id in meas_qs for o in goal.objectives)
    # description mentions derivation
    assert "derived" in goal.description or "quantities" in goal.description
