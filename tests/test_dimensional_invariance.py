...
Now also covers the pure dimensionless security (NIST crypto sizing) validators.
...
HOMOGENEOUS_CHECKS = [
    ...
    # security validators: pure math on dimensionless bit/invocation counts (no baked dimensional const)
    ("birthday_bound", birthday_bound_check, {
        "space_bits": (96.0, "1"), "n_uses": (float(2 ** 32), "1")}),
    ("gcm_invocation_budget", gcm_invocation_budget_check, {
        "n_invocations": (float(2 ** 30), "1")}),
]
