...
# --- acceptance: a drone Specification auto-fires the flight axes through the gate -------
# Mirrors the pattern in test_dynamics.py / test_security.py for full integration symmetry.
# (Detailed drone coverage also lives in test_physics_selection.py; this ensures the dedicated
# flight test file exercises the recipe/gate wiring end-to-end.)
...
def _drone_spec() -> Specification:
    """Minimal drone that declares all flight triggers + needed inputs (units as in recipes)."""
    qs = [
        ...
        _q("cah", 1.3, "Ah", "battery.capacity_ah"),
    ...
    return Specification(run_id="drone", idea="quadrotor", quantities=qs)


def test_flight_axes_auto_select_and_units_resolve():
    """The measurand tags alone select all four flight validators; units convert soundly."""
    checks, gaps = select_physics_checks(_drone_spec())
    assert {"rotor_hover", "battery_endurance", "current_budget", "attitude_pd"} <= {c.validator for c in checks}
    assert gaps == []


def test_drone_with_good_numbers_passes_gate():
    """A drone whose declared flight physics all clear yields physics_ok."""
    res = evaluate_spec_physics(_drone_spec())
    assert res["gate"].passed and res["gaps"] == []
    assert len(res["checks"]) == 4


def test_underpowered_drone_fails_gate_on_rotor_hover():
    """Insufficient thrust (below 2:1) surfaces as PHYSICS_CHECK_FAILED, not a masked pass."""
    bad = _drone_spec()
    # mutate max thrust down
    for i, q in enumerate(bad.quantities):
        if q.measurand == "rotor.max_total_thrust":
            bad.quantities[i] = _q(q.id, 15.0, q.unit, q.measurand)  # type: ignore[attr-defined]
            break
    res = evaluate_spec_physics(bad)
    assert not res["gate"].passed
    assert any("rotor_hover" in f.detail for f in res["gate"].failures)
