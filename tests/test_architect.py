...
def test_assembles_specification_that_passes_gate_gamma():
    ...
    # γ+ wire exercised (guarded attach; now *real* InverseDesignGoal derived from spec.quantities/measurands, no dummies)
    assert state.pareto_front is not None
    assert state.pareto_front.goal is not None
    # wiring proof: goal objectives reference real quantity ids from this spec (not "nonexistent")
    spec_qids = {q.id for q in spec.quantities}
    goal_qids = {o.quantity_id for o in state.pareto_front.goal.objectives}
    assert goal_qids, "real goal must have >=1 objective"
    assert goal_qids.issubset(spec_qids) or (len(goal_qids) == 1 and next(iter(goal_qids)) in spec_qids), (
        f"goal qids {goal_qids} must come from spec qs {spec_qids}"
    )
    assert "derived" in state.pareto_front.goal.description.lower() or "quantities" in state.pareto_front.goal.description.lower()
