...
def test_formula_helpers_are_exact():
    ...
    assert axial_stress_formula("q_force", "q_area") == "q_force / q_area"
...
def test_axial_stress_is_pressure_dimension():
    # σ = F / A : force/area = pressure (verified by C-15)
    dims = {"q_force": parse_unit("N"), "q_area": parse_unit("mm^2")}
    assert formula_dimension(axial_stress_formula("q_force", "q_area"), dims) == parse_unit("MPa")
