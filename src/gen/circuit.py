...
Wiring (physics validators campaign): core solver for "circuit" layer (electronics + elektriker pipeline DC operating point). Used by electronics.run_electronics_simulation, pipelines/elektriker (map + sim), thermal (power), simulation/runner, reality (falsification). Seams to gate_erc (before sim), cad/assembly. Advances Step 7 physics/electronics validators + Step 8-9 pipelines.
"""
...
def _validate_components(components):
    """Fail-loud on non-positive or invalid component params (prevents div0/singularity).
    Consistent with fem.py guards and project style for physics/electronics layers.
    """
    for c in components:
        if isinstance(c, Resistor) and c.ohms <= 0:
            raise ValueError(f"Resistor ohms must be >0 (got {c.ohms})")
        if isinstance(c, Capacitor) and c.farads <= 0:
            raise ValueError(f"Capacitor farads must be >0 (got {c.farads})")
        if isinstance(c, Inductor) and c.henries <= 0:
            raise ValueError(f"Inductor henries must be >0 (got {c.henries})")
        if isinstance(c, Diode) and c.i_sat <= 0:
            raise ValueError(f"Diode i_sat must be >0 (got {c.i_sat})")
