...
def contact_check(max_pressure: float, allowable_pressure: float) -> dict:
    """Contact-pressure design check.

    Compares the computed peak Hertz pressure to an `allowable_pressure` (MPa, e.g. a
    material's allowable contact stress / a fraction of its hardness). Returns
    ``{"max_pressure", "allowable_pressure", "safety_factor", "ok"}``: safety_factor =
    allowable/max, ok = safety_factor >= 1. Deterministic. Raises ValueError on a
    non-positive pressure (a peak pressure of zero means no contact)."""
    if max_pressure <= 0.0:
