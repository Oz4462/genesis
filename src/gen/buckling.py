...
FEM cross-check (via fem.beam_element_stiffness + geometric stiffness eigen): the
computed P_cr converges to the closed-form euler_critical_load for all 4 end conditions.
This is defense-in-depth (two independent methods agreeing). See fem.py and buckling
eigen code.

Wiring (physics validators campaign): registered as "buckling" in physics_validation.VALIDATORS;
auto-selected by physics_selection recipe "column buckling" (trigger="column.axial_load").
Invoked via gate_delta_physics in pipeline.assess_specification for honest physics_ok.
Cross-checks fem + closed-form + structural. L3 seam to selection/gate/pipeline.
