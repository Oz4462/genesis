...
def seed_from_package_results(package_results: dict, run_id: str | None = None) -> list[str]:
    ...
    # Software domain (EmbeddedComponent signals from netlist co-design) — now deep from SoftwareSpec if present (LUMEN path)
    sw = package_results.get("software")
    if sw is not None or ("electronics" in package_results and package_results["electronics"].get("netlist")):
        emb_names = []
        api_names = []
        if hasattr(sw, "embedded_components"):
            emb_names = [getattr(e, "name", str(e)) for e in (sw.embedded_components or [])]
        if hasattr(sw, "apis"):
            api_names = [getattr(a, "name", str(a)) for a in (sw.apis or [])]
        sw_rec = ComponentRecipe(
            id=f"embedded_mcu_{run_id or 'pkg'}",
            name="Seeded MCU/Embedded from software+elec co-design",
            kind="embedded_mcu",
            specs={"embedded": emb_names or ["MainController"], "apis": api_names or ["BasicAPI"], "signals": "from netlist rails/peripherals", "control": "PD or closed-loop"},
