...
def build_full_mini_realization_package(ideas: list[str], package_name: str = "Jetpack Full Mini Package", run_id: str = None) -> str:
    ...
    try:
        sw_spec = map_to_software_spec(c, i, run_id=run_id) if 'c' in locals() and 'i' in locals() else None
        if sw_spec:
            software_report = {
                "embedded": [e.name for e in getattr(sw_spec, "embedded_components", [])],
                "apis": [a.name for a in getattr(sw_spec, "apis", [])],
                "update_rollback": getattr(getattr(sw_spec, "update_pfad", None), "rollback", "n/a"),
    ...
    cert_report = {"note": "certs via LUMEN RunState (full δ+γ+εζΩ + omega) for realize; see lumencrucible:500 post-certs + state.py:1327. Full consumer support wired (bundle/web/cli/pipeline/integrator) — CONSUMERS FULL CERTS 4L"}
    try:
        ...
        if ideas:
            ...
            cert_report = {
                ...
                "has_omega_certificate": getattr(rs, "omega_certificate", None) is not None if rs else False,
                ...
                "has_pareto_front": getattr(rs, "pareto_front", None) is not None if rs else False,
                "has_coverage_certificate": getattr(rs, "coverage_certificate", None) is not None if rs else False,
                "has_reality_verdict": getattr(rs, "reality_verdict", None) is not None if rs else False,
                "has_delta_plus_result": getattr(rs, "delta_plus_result", None) is not None if rs else False,
                "source": "LUMEN.process_dream (integrator full E2E cert to RunState)",
            }
            # lightweight + full rs for MAX AGENTS / consumers
            if sc:
                cert_report["seam_certificate"] = {"complete": getattr(sc, "complete", False), "produced_by": getattr(sc, "produced_by", "seams")}
            if mf:
...
def _generate_software_stub(pkg_root, sw_report: dict, run_id: str | None):
    ...
    lines.append("- Embedded from control signals (Elektriker) + thermal constraints (DFM)")
    lines.append("- Testplan seeds from Lern fault injection + safety ladder")
    lines.append("- OTA/rollback from Techniker maintenance")
    lines.append("- Executable validation: via gen.software.run_python_artifact + gate_code (on CodeArtifact in full Spec)")
    lines.append("")
    lines.append("**Gap:** Full firmware source gen + HIL integration + OTA package in later stone. This is the design spec first stone.")
    lines.append("SoftwareSpec is also reachable via LUMEN multi_domain for complex (drone/robot) + Closed-Loop seed.")
...
def realize(ideas: list[str], package_name: str = "Genesis Realization Package", run_id: str = None) -> dict[str, str]:
    ...
    try:
        ...
        if ideas:
            ...
            if run_state is not None and certs_from_manifest and isinstance(certs_from_manifest, dict):
