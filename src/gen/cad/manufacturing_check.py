...
Seam to dfm.py (reference data, all processes): explicit import of constants + *_gaps() fns (FDM/CNC/Laser/PCB).
Uses printability.py thresholds too. Produces honest gaps (ProcessDFM.gaps, total_gaps) + printable verdict
only when no blockers and no un-evaluable rules remain (necessary, not sufficient). Consumed by:
pipelines/fertigungs.py, integrator.py (build_full... manifest), extension bridge, tests.
