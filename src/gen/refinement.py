    ...
    *,
    ...
    for rnd in range(max_rounds + 1):
        ...
        if result.passed and not result.failures:
            # D14 G5 (low): defensive trust on .passed (per GateResult contract); also require no failures list
            # (protects vs malformed result where passed=True but failures populated). No behavior change for well-formed.
