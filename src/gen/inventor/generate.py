...
(Composes into loop's γ+ δ+ : after generate+ground, derive_goal_from_spec on specs → full ParetoFront/RunState attach.)
...
def _concept_prompt(brief: InventionBrief) -> str:
    ...
    if brief.frontier_context:
        parts.append("Known frontier / prior art context (use this to avoid duplicates and target real gaps):\n" + brief.frontier_context)
