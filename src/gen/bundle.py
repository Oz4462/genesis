...
class BundleManifest:
    ...
    printed_share: float
    spec_gaps: list[str]
    # Full δ+/γ+/Ω cert support for consumers (gap#7 close). Honest None on assess-path (no RunState);
    # richer when attached upstream. See CONSUMERS FULL CERTS 4L.
    seam_certificate: dict | None = None
    memory_fabric: dict | None = None
    pareto_front: dict | None = None
    omega_certificate: dict | None = None
...
def emit_bundle(spec: Specification, out_dir: str | Path, *, tolerance: float = 0.1) -> BundleManifest:
    ...
    manifest = BundleManifest(
        ...
        # E2E HORIZON certs (from Assessment; full δ+/γ+/Ω for consumers)
        # Honest: δ+/γ+/Ω are None on pure assess (seam/mem always here); richer on RunState/LUMEN.
        # Closes gap#7 E2E asymmetry for bundle consumers. See CONSUMERS FULL CERTS 4L.
        seam_certificate=(
            {"has_seams": bool(assessment.seam_certificate.seams), "num_seams": len(assessment.seam_certificate.seams), "complete": assessment.seam_certificate.complete}
            if assessment.seam_certificate else None
        ),
        memory_fabric=(
            {"num_deposits": len(assessment.memory_fabric.deposits), "num_recalls": len(assessment.memory_fabric.recalls), "health": getattr(assessment.memory_fabric.health, "value", None)}
            if assessment.memory_fabric else None
        ),
        pareto_front=(
            {"n_candidates": len(assessment.pareto_front.candidates), "n_evaluated": len(assessment.pareto_front.evaluated_candidates)}
            if getattr(assessment, "pareto_front", None) else None
        ),
        omega_certificate=(
            {"n_receipts": len(getattr(assessment.omega_certificate, "gate_receipts", []) or [])}
            if getattr(assessment, "omega_certificate", None) else None
        ),
        coverage_certificate=(
            {"complete": getattr(assessment.coverage_certificate, "complete", None)}
            if getattr(assessment, "coverage_certificate", None) else None
        ),
        reality_verdict=(
            {"status": getattr(assessment.reality_verdict, "status", None), "within_tolerance": getattr(assessment.reality_verdict, "within_tolerance", None)}
