...
D11/D12 (low closed): audit logs for _queries/_check , union verification sources
(dedup URL), inter-judge family asserts.
"""
...
class Skeptic:
    ...
    async def run(self, state: RunState) -> RunState:
        for claim in state.claims:
            ...
            # D12 (inter-judge Familien-Dedup supplement to D7): ensure judges mutually different families
            # (not just vs generator). Previously only vs-gen asserted.
            if self._second is not None:
                assert_different_families(self._verifier.model, self._second.model)
            for ex in self._extra:
                assert_different_families(self._verifier.model, ex.model)
                if self._second is not None:
                    assert_different_families(self._second.model, ex.model)
            ...
            if self._extra:
                ...
                for ex in self._extra:
                    ...
                    extra_verdict_lists.append(ex_ver)
                    judgments.append(self._aggregate(ex_ver, ex.model))
            ...
            # D11: claim.verification now unions Primary + Second + Extra sources (dedup by URL)
            # Previously only primary (missing Second/Extra in audit trail for panel case).
            all_vs = list(verifier_verdicts) + list(second_verdicts)
            for evl in extra_verdict_lists:
                all_vs.extend(evl)
            seen_u: set[str] = set()
            verification = []
            for v in all_vs:
                if v.relation != "irrelevant" and v.url not in seen_u:
                    seen_u.add(v.url)
                    verification.append(self._fetch_ref(v))
    ...
    async def _independent_candidates(self, claim_text, scholar_urls, state):
        seen: set[str] = set()
        out = []
        for query in await self._check_queries(claim_text, state=state):
            for backend in self._backends:
                ...
                for c in cands:
                    ...
                    out.append(c)
        # D12: dedup is exact url_or_id (backend provided). Mirrors/CDN same-content
        # may appear distinct; content_hash/canonical would need post-fetch (costly).
        # + scholar exclusion is the independence guarantee. Sufficient for low D.
        return out

    async def _check_queries(self, claim_text: str, *, state: RunState | None = None) -> list[str]:
        ...
        try:
            ...
            # D11: now log swallowed LLM/parse error to state for audit trail (was silent []).
            if state is not None:
                state.log.append(f"skeptic: LLM/parse error in _check_queries for claim: {type(exc).__name__} (best-effort)")
            queries = []
    ...
    async def _judge(self, llm: LLMClient, claim_text: str, content: str, url: str) -> _Verdict:
        ...
        try:
            ...
            # D11 note: per-source LLM/parse error -> irrelevant/0 (conservative, no fabrication).
            # Not individually logged here (would be noisy for every source); surfaced via
            # missing verification entry + general logs. Full audit via state + ledger.
