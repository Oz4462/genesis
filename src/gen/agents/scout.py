...
class Scout:
    ...
    async def run(self, state: RunState) -> RunState:
        ...
        for sq in focuses:
            for query in await self._queries(sq.text, state=state):
    ...
    async def _queries(self, focus: str, *, state: RunState | None = None) -> list[str]:
        ...
        try:
            ...
            # D11 (audit logs low): now logs LLM/parse swallow to state (reproducible trail).
            # Previously silent return. state param keeps fn usable in tests without state.
            if state is not None:
                state.log.append(f"scout: LLM/parse error in _queries for {focus!r}: {type(exc).__name__} (best-effort)")
            return queries
