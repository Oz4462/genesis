        ...
        if rad_trigger:
            ...
            if absorbed is None or eps is None or area is None or t is None:
                ...
                try:
                    ...
                    # co-sim coupling: callers combine `net` with
                    # co_sim_electronics_thermal(radiation_net_w=net) — see runtime_notes.
                    # (A previous inline example referenced an undefined `base_artifact`
                    # and could never execute; removed as dead code, ruff F821.)
                except Exception:
