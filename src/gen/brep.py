...
Key public API:
  csg_to_solid(node, quantities) -> Solid
  exact_volume(node, quantities) -> float
  is_valid(node, quantities) -> bool
  interferes(node_a, node_b, quantities, tolerance=1e-9) -> bool
...
Integrations (wiring):
- geometry_verification.verify_geometry (Tier-3 cross-check BREP vs analytic AABB/volume; D15 in pipeline)
- export.brep_stl (specification_to_brep_stl / component_to_brep_stl for print-ready tessellated STL with booleans)
- orientation (overhang/bridge/first_layer via tessellation on real solid)
- pipeline.assess_printability (geo_verif + brep_stl + stl_integrity + blockers)
- bundle / cli (component/spec STL export paths)
- tests (test_brep, test_geometry_verification, test_brep_stl, test_mesh_integrity, test_rotate)
