...
Wiring & Consumers (explicit seams, per 4_LINSEN L3 + GENESIS_PLATFORM_PLAN §3.6 CAD/CAE/Fertigung Kern + §4.7 Fertigungs-Pipeline):
- Primary CAD path: gen.cad.manufacturing_check.check_advanced_dfm + check_manufacturing
  (imports all FDM/CNC/Laser/PCB constants + *_gaps() fns; produces ProcessDFM.gaps + total_gaps;
   printable only if no issues AND no gaps; used by pipelines/integrator.py:240, fertigungs.py:256 via dfm_report).
- Physics/δ-gate path: gen.future_ideas._dfm_quantities + competitive_humanoid + demo (min_wall/hole as
  derived + constraints); consumed by test_dfm.py (gate_gamma via verification/gates) + physics_validation
  (indirect via printability.py subset of rules; dfm.py is the FDM reference data layer).
- Realization: pipelines/fertigungs.py (_fdm_cost... + DFM ref in FertigungsSpec), integrator.build_* (manifest
  advanced_dfm + gcode_program), cad/__init__.py reexports, extensions/breakthrough_bridge.
- gcode/cost seams: cad.gcode (CNC profile), cad.cost_model (FDM only; CNC/Laser/PCB gaps declared).
Never silent pass on un-evaluable (CNC geometric, Laser profile, PCB copper, FDM hole geom).
