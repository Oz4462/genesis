...
def _direction_from(q: "Quantity") -> ObjectiveDirection:
    """Heuristic direction for a real objective derived from a spec Quantity.
    MAXIMIZE for performance/strength-like (hints in id/name/measurand); else MINIMIZE (conservative default).
    Used only for γ+ real-goal derivation; explicit goals (e.g. tests) override.
    """
    text = f"{q.id} {q.name} {q.measurand or ''}".lower()
    maximize_hints = (
        "effic", "strength", "stiff", "modul", "freq", "speed", "perform",
        "margin", "safety", "torque", "force", "yield", "stiffness",
    )
    if any(h in text for h in maximize_hints):
        return ObjectiveDirection.MAXIMIZE
    return ObjectiveDirection.MINIMIZE


def derive_goal_from_spec(
    spec: Specification,
    goal_id: str,
    description: str | None = None,
) -> InverseDesignGoal:
    """Derive *real* InverseDesignGoal from architect (or other) spec's quantities/measurands.

    - quantity_id and unit come directly from existing Quantity entries (never invented).
    - Prefers quantities declaring a measurand (per HORIZON + measurand patterns in physics_selection/costing/score).
    - At most 3 objectives (small, realistic Pareto dimension).
    - Direction heuristic from keywords (see _direction_from); default MINIMIZE.
    - If no quantities (e.g. lumen-small skeleton): produces a goal whose objective will cause
      honest OBJECTIVE gap in build, but uses real ctor + description (no dummy/placeholder text).
    Guarded callers: never raises on empty; pure deterministic.
    """
    if description is None:
        description = "real objectives derived from spec quantities/measurands (γ+)"
    qs = list(spec.quantities or [])
    # prioritize measurand-bearing (explicit declared physicals)
    prioritized = [q for q in qs if getattr(q, "measurand", None)] + [
        q for q in qs if not getattr(q, "measurand", None)
    ]
    ...
    for idx, q in enumerate(selected):
        objectives.append(
            DesignObjective(
                id=f"obj_{idx}_{q.id}",
                quantity_id=q.id,
                direction=_direction_from(q),
                unit=q.unit,
            )
        )
    if not objectives:
        # honest skeleton path (lumen small): real goal structure; will gap on missing qid (expected)
        objectives = [
            DesignObjective(
                id="demo_obj",
                quantity_id="lumen_demo_q_skeleton",
