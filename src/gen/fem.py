...
# Cross-check usage: buckling.py uses beam_element_stiffness + geometric for eigen P_cr,
# compared to euler_critical_load. See physics_validation "buckling" validator.
...
def beam_element_stiffness(e_modulus: float, inertia: float, length: float) -> np.ndarray:
    ...
    *Concepts and Applications of Finite Element Analysis*)."""
    if e_modulus <= 0.0 or inertia <= 0.0 or length <= 0.0:
        raise ValueError("e_modulus, inertia and length must be positive")
    L = float(length)
