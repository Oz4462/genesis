...
RECIPES: list[CheckRecipe] = [
    ...
    # Thermal mismatch (bonded materials with different CTE under delta_t)
    # Makes the existing thermal_mismatch_check auto-selectable from specs.
    CheckRecipe(
        name="thermal mismatch (bonded materials)",
        validator="thermal_mismatch",
        trigger="thermal.delta_t",
        inputs={
            "e1": ("material1.elastic_modulus", "MPa"),
            "area1": ("material1.cross_section_area", "mm^2"),
            "cte1": ("material1.cte", "1/K"),
            "e2": ("material2.elastic_modulus", "MPa"),
            "area2": ("material2.cross_section_area", "mm^2"),
            "cte2": ("material2.cte", "1/K"),
            "delta_t": ("thermal.delta_t", "K"),
            "strength1": ("material1.ultimate_strength", "MPa"),
            "strength2": ("material2.ultimate_strength", "MPa"),
        },
    ),
    # Overtemperature via conduction rise (power -> ΔT -> peak vs material limit).
    # Makes overtemperature_check auto-selectable (was registered in VALIDATORS but missing from RECIPES -> L3 seam gap).
    CheckRecipe(
        name="overtemperature (conduction)",
    ...
    # Fracture (linear elastic, fast fracture from crack). Makes fracture_check auto-selectable.
    # (paris_life for growth is in module but not yet a registered validator check)
    CheckRecipe(
        name="fracture (fast fracture)", validator="fracture", trigger="flaw.crack_length",
        inputs={
            "stress": ("flaw.stress", "MPa"),
            "crack_length_a": ("flaw.crack_length", "mm"),
            "fracture_toughness_kic": ("material.fracture_toughness", "MPa*sqrt(mm)"),
        },
        extra={"geometry_factor_y": 1.12},
    ),
    # Plate bending (circular, uniform pressure, clamped or simply supported). Makes plate_bending_check auto-selectable.
    CheckRecipe(
        name="plate bending", validator="plate_bending", trigger="plate.pressure",
        inputs={
            "pressure_q": ("plate.pressure", "MPa"),
            "radius_R": ("plate.radius", "mm"),
            "thickness_t": ("plate.thickness", "mm"),
            "e_modulus": ("material.elastic_modulus", "MPa"),
            "nu": ("material.poisson", "1"),
            "allowable_stress": ("material.allowable_stress", "MPa"),
        },
        extra={"edge": "clamped"},
    ),
    # Bolted joint (preloaded concentric load sharing). Makes bolted_joint_check auto-selectable.
    CheckRecipe(
        name="bolted joint (preloaded load sharing)", validator="bolted_joint", trigger="joint.external_load",
        inputs={
            "torque": ("joint.tightening_torque", "N*mm"),
            "nominal_diameter": ("bolt.diameter", "mm"),
            "tensile_stress_area": ("bolt.tensile_area", "mm^2"),
            "external_load_P": ("joint.external_load", "N"),
            "bolt_stiffness_kb": ("joint.bolt_stiffness", "N/mm"),
            "member_stiffness_km": ("joint.member_stiffness", "N/mm"),
            "proof_strength": ("bolt.proof_strength", "MPa"),
        },
        extra={"k_factor": 0.2},
    ),
    # Creep-rupture life at high temp (Larson-Miller). Makes creep_life_check auto-selectable.
    CheckRecipe(
        name="creep-rupture life", validator="creep", trigger="creep.design_life",
        inputs={
            "applied_stress": ("creep.stress", "MPa"),
    ...
    # Hertzian contact (sphere/cylinder). Makes contact check auto-selectable.
    # Updated recipe to supply allowable + geometry so validator computes p0 internally.
    # Matches contact_check signature (supports geometry path + allowable).
    # L3 seam fix (per harness 4L review): trigger now "contact.force" (the actual load measurand)
    # matching catalog convention used by every other recipe (shaft.torque, column.axial_load, ...).
    CheckRecipe(
        name="hertzian contact stress", validator="contact", trigger="contact.force",
        inputs={
            "force": ("contact.force", "N"),
            "r1": ("contact.radius1", "mm"),
            "r2": ("contact.radius2", "mm"),
            "e1": ("material1.elastic_modulus", "MPa"),
            "nu1": ("material1.poisson", "1"),
            "e2": ("material2.elastic_modulus", "MPa"),
            "nu2": ("material2.poisson", "1"),
