...
# costing/export seam (Step 8-9): uses bom_cost (from costing.py: priced BOM + fabricated filament via volume*density*price) + format_cost.
# Surfaces δ geometry gate (gate_delta from verification.gates, not full physics δ+). Complements cad/cost_model (FDM ranged process est via mfg/fertigungs).
# Consumers: bundle (BAUANLEITUNG.md + MANIFEST cost), cli (render + print), web (via assess path). Wiring proven: markdown:19+111, costing:63, seams:231 (ε cost_rollup), pipeline:130 (completeness).
