...
def _check_solution_finite(u: np.ndarray) -> None:
    ...
    direct solve) → GeometryError, the class modal.py/buckling.py raise for a
    degenerate structure. Never return NaN silently."""
...
def solve_elasticity(
    ...
    non-finite displacement field (degenerate mesh / ill-posed BCs) — see
    ``_check_material_and_bcs`` / ``_check_solution_finite``.
    """
